...
for(var B=0;
B<C.length;
B++){C[B].down("a").observe("click",function(D){new Ajax.Request(D.currentTarget.href+"&xpage=createinline&ajax=1",{method:"get",onSuccess:function(F){var E=F.getHeader("redirect");
...
var B=this;
if(C.nodeName!="A"){C=$(button).firstChild
...
XWiki.Document.URLTemplate="/xwiki/bin/__action__/__space__/__page__";
...
document.observe("xwiki:dom:loaded",function(){var D={documents:{script:XWiki.Document.getRestSearchURL("scope=name&number=10&media=json&"),varname:"q",icon:"/xwiki/resources/icons/silk/page_white_text.gif",noresults:"Document not found",json:true,resultsParameter:"searchResults",resultId:"id",resultValue:"pageFullName",resultInfo:"pageFullName"},spaces:{script:XWiki.Document.getRestSearchURL("scope=spaces&number=10&media=json&"),varname:"q",icon:"/xwiki/resources/icons/silk/folder.gif",noresults:"Space not found",json:true,resultsParameter:"searchResults",resultId:"id",resultValue:"space",resultInfo:"space"},users:{script:XWiki.currentDocument.getURL("get","xpage=uorgsuggest&classname=XWiki.XWikiUsers&wiki=local&uorg=user&"),varname:"input",icon:"/xwiki/resources/icons/silk/user.gif",noresults:"User not found"},groups:{script:XWiki.currentDocument.getURL("get","xpage=uorgsuggest&classname=XWiki.XWikiGroups&wiki=local&uorg=group&"),varname:"input",icon:"/xwiki/resources/icons/silk/group.gif",noresults:"Group not found"}};
